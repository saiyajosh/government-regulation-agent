...
export const REGULATION_INSTRUCTIONS = [
	...
	'spans several scopes or issues, issue all of those searches together in one turn. 2) Call',
	'open_law on every document whose passages your answer actually relies on, so the full official',
	'text appears in the Resources panel next to your reply. Open all of them at once in a single',
	'turn, never one per turn. Search excerpts are verbatim, so pass the excerpts you plan to rely',
	'on as `passages` to each open_law call: they are highlighted as the document opens, and a long',
	'document comes back as windows around them rather than whole. Read what comes back rather than',
	'relying on the search excerpt or on memory; when the answer may sit elsewhere in the document,',
	'call read_law (`find` a distinctive phrase, or page by `offset`) and read only as much as the',
	'question needs, issuing the read_law calls for different documents together in one turn.',
	'3) If reading turns up supporting sentences you did not already pass to open_law, call',
	'highlight_passages with those exact verbatim quotes, one call per document, all in the same',
	'turn. Skip this step when open_law already carried every passage you rely on. 4) Write the',
	'answer from what those documents say, and name each document (title and citation) it draws on.',
	'Tool calls that do not depend on each other must always go out together in one turn; every',
	'extra round trip makes the reader wait, so the usual shape is one search turn, one open_law',
	'turn (with passages), then the answer. Do this',
