...
import { ask, openedKeys, RegulationEval } from './harness.ts';
...
it('says plainly when the library does not cover the question instead of answering from memory', async () => {
	...
	// The gap is stated about San Jose specifically, and nothing was opened
	...
	expect(openedKeys(toolCalls).every((key) => FIXTURES.some((doc) => doc.key === key))).toBe(true);
});
...
it('answers a question far outside the library without inventing a source', async () => {
	...
	expect(toolCalls.map((call) => call.name)).not.toContain('open_law');
